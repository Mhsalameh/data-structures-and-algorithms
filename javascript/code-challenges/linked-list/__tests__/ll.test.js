...
const { LinkedList, zipLists } = require('../ll.js');
...
describe('testing linkedlist', () => {
  ...
  it('testing kthFromEnd', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      `argument:'k' can't be equal or more than 1: length of the list, k is limited from 0 to 0`
    );
    list.append('b');
    kthValue = list.kthFromEnd(1);
    expect(kthValue).toEqual('a');
  ...
  it('testing Linked list zip', () => {
    let list1 = new LinkedList();
    let list2 = new LinkedList();
    let list3 = new LinkedList();
    list1.append('a');
    expect(zipLists(list1, list2).toString()).toEqual('{a}-> null');
    list2.append('b');
    expect(zipLists(list1, list2).toString()).toEqual('{a}-> {b}-> null');
    list2.append('c');
    ...
    expect(zipLists(list3, list1).toString()).toEqual('{a}-> {d}-> null');
  });
});
